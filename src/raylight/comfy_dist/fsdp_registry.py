...
class FSDPShardRegistry:
    ...
    @classmethod
    def wrap(cls, model, fsdp_state_dict=None, cpu_offload=False, expert_parallel=False):
        ...
        for registered_cls, shard_func in cls._REGISTRY.items():
            if isinstance(model, registered_cls):
                ...
                return shard_func(model, fsdp_state_dict, cpu_offload, expert_parallel)
...
if hasattr(model_base, "WAN21"):
    ...
    def _wrap_wan(model, sd, cpu_offload, ep):
        return wan_shard(model, sd, cpu_offload, ep)

if hasattr(model_base, "ChromaRadiance"):
    @FSDPShardRegistry.register(model_base.ChromaRadiance)
    def _wrap_chroma_radiance(model, sd, cpu_offload, *args):

...
def cfg_parallel_forward_wrapper(executor, *args, **kwargs):
    ...
    # x, timesteps, context, attention_mask, guidance, ref_latents, transformer_options
    ...
    if ref_latents is not None:
        if isinstance(ref_latents, (list, tuple)):
            ref_latents = [torch.chunk(r, cfg_world_size, dim=0)[cfg_rank] for r in ref_latents]
        else:
